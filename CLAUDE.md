...
- **No secrets in source files** — no API keys, tokens, passwords, or credentials hardcoded anywhere in `src/`, `server/`, `src-tauri/`, or config files. All secrets must live in `.env` (which is gitignored).
- **`.gitignore` is respected** — confirm `.env`, `*.db`, `*.db-shm`, `*.db-wal`, `images/`, `tracks/`, `releases/`, and `src-tauri/binaries/` are not staged or tracked.
