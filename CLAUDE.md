...
This repo is dev-only. The actual PWA your phone connects to is a separate checkout at `C:\Users\Bperk\troubadour`, run under PM2 (process name `troubadour`, port 5401) alongside an unrelated app (`budget-hero`, port 5400) on the same PM2 daemon. **After every `git push` to `main` in this session, you must also bring that checkout up to date before ending the session:**
...
6. Verify the fix actually landed, e.g. `curl -s http://localhost:5401/` and check for the expected content.
